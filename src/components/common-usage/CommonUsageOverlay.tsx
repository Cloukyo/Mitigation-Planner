...
import { COMMON_USAGE_UPDATED_EVENT, getPlanCommonUsage, updateCommonUsageLayer, updateExternalActionUsage } from "@/lib/common-usage/commonUsageStorage";
...
export function CommonUsageOverlay({
  encounter,
  planId,
  players,
  placements,
  ...
  function selectedLayer() {
    return selected ? layers.find((layer) => layer.sourceId === selected.sourceId) : undefined;
  }
    ...
    <>
      {visible.map(({ usage, layer }) => (
        <CommonUsageMarker
          ...
          onSelect={(next) => {
            const suggested = suggestedPlayersForUsage(next, players);
            setSelectedPlayerId(suggested[0]?.id ?? players[0]?.id ?? "");
            setSelected(next);
          }}
        />
      ))}
      <CommonUsageBottomSheet
        ...
        onIgnore={() => {
          ...
          if (layer) updateCommonUsageLayer(layer);
          setSelected(null);
        }}
      />
    </>
